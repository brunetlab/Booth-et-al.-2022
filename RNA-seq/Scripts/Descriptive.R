...
#################################################################
legend <- unique(pData(eset)[, c("Presence.of.males", "Age", "col", "bg", "Days.with.Males", "Sex", "shp")])
...
legend(x=100, y=0,
       ...
       pch = legend$shp,
       pt.cex = 1.3,
       legend = paste("day", legend$Age, legend$Sex, "(+", legend$Presence.of.males, "for", legend$Days.with.Males, "days)"),
       bty = "n")
dev.off()
