...
#################################################################
legend <- unique(pData(eset)[, c("Presence.of.males", "col", "bg", "shp")])
# legend <- legend[c(2,3,1,6,5,7,4), ]

pdf("Results/pca_vst_day3.pdf",
...
legend(x=60, y=0,
       ...
       pch = legend$shp,
       pt.cex = 1.3,
       legend = paste("day 3 her (+", legend$Presence.of.males, "for 1 day)"),
       bty = "n")
dev.off()

pdf("Results/pca_vst_day3_PC2-3.pdf",
    width = 6.0, height = 3.2)
par(mar = c(4.1, 4.1, 1.1, 15.1),
    xpd = TRUE)
plot(pca$x[, "PC2"],
...
pdf("Results/pca_vst_batch_day3.pdf",
